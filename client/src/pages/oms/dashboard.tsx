...
export default function OmsDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { activeOrders: activeOrders.length, pendingDispatch: pendingDispatch.length, inTransit: inTransit.length, lowCritical: lowCritical.length, fulfillmentRate };
  }, []);
  ...
  const recentShipments = useMemo(() =>
    omsShipments.filter(s => s.status !== "rto" && s.status !== "lost").slice(0, 10),
  []);
          ...
          <Fade>
            <div className="border border-border rounded-xl bg-background overflow-hidden">
                  ...
                  return (
                    <div key={type}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-muted-foreground">{count} orders · ₹{(value / 1000).toFixed(0)}K</span>
                      </div>
              ...
              <div className="border-t border-border p-4">
                <h3 className="text-xs font-semibold text-muted-foreground mb-3">Recent Dispatches</h3>
                <div className="space-y-2">
                  {recentShipments.slice(0, 6).map((shp) => (
                    <div key={shp.id} className="flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded shrink-0", courierBadgeColors[shp.courier])}>
                          {shp.courier.substring(0, 3).toUpperCase()}
                        </span>
                        <span className="font-mono text-[10px] text-muted-foreground truncate">{shp.awbNumber}</span>
                      </div>
                      <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full shrink-0", shipStatusColor[shp.status])}>
                        {shp.status === "in-transit" ? "Transit" : shp.status === "out-for-delivery" ? "OFD" : shp.status}
                      </span>
                    </div>
                  ))}
                </div>
