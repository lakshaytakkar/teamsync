...
const statusStyle: Record<string, string> = {
  ...
  "action-required": "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400",
};
                ...
                <Card key={doc.id} className="flex items-center gap-4 p-4 hover:shadow-sm transition-shadow" data-testid={`doc-${doc.id}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-muted-foreground">·</span>
                      <Badge className={cn("text-[10px] border-none", statusStyle[doc.status])}>
                        {doc.status === "pending-review" ? "Pending Review" : doc.status === "action-required" ? "Action Required" : "Verified"}
                      </Badge>
                    </div>
                  </div>
